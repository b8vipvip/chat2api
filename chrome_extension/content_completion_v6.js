(() => {
  const KEY = "__CHAT2API_COMPLETION_RECOVERY_V6__";
  ...
  function transientStatusVisible() {
    ...
    return candidates.some(node => {
      ...
      return /(^|\s)(thinking|analyzing|reasoning|searching|browsing|working)(\s|$)|正在(思考|分析|推理|搜索|浏览|处理)/i.test(text);
    });
  }

  async function emitDiagnostics(requestId, strategy, stableMs, actionCount, button) {
    try {
      await chrome.runtime.sendMessage({
        ...
        event: {
          ...
          diagnostics: {
            request_controller_overlay: "completion-recovery-v6",
            completion_recovery: strategy,
            completion_stable_ms: stableMs,
            completion_final_action_count: actionCount,
            completion_stale_stop_label: normalize(`${button?.dataset?.testid || ""} ${button?.getAttribute?.("aria-label") || ""}`).slice(0, 120),
          },
        },
      });
    } catch (_) {}
  ...
  function restoreSuppressedButton() {
    ...
    const previous = button.dataset.chat2apiPreviousVisibility;
    if (previous === "__empty__") button.style.removeProperty("visibility");
    else if (previous != null) button.style.visibility = previous;
    delete button.dataset.chat2apiPreviousVisibility;
    delete button.dataset.chat2apiStaleStopSuppressed;
  }

  async function suppressStaleStop(active, button, strategy, stableMs, actionCount) {
    if (!button || button.dataset.chat2apiStaleStopSuppressed === "v6") return;
    button.dataset.chat2apiPreviousVisibility = button.style.visibility || "__empty__";
    button.dataset.chat2apiStaleStopSuppressed = "v6";
    button.style.visibility = "hidden";
    state.suppressedButton = button;
    await emitDiagnostics(active.requestId, strategy, stableMs, actionCount, button);
    state.restoreTimer = setTimeout(restoreSuppressedButton, 2600);
  }
  ...
  async function tick() {
    ...
    if (!active?.requestId || active.cancelled) {
      if (state.requestId && state.suppressedButton) restoreSuppressedButton();
    ...
    if (!button || !state.stableSince) return;
    const stableMs = Date.now() - state.stableSince;
    const actions = finalActionControls(latest);
    if (actions.length && stableMs >= 2500) {
      await suppressStaleStop(active, button, "stale-stop-final-actions", stableMs, actions.length);
      return;
    }

    // Conservative fallback for UI variants that do not expose the final action row.
    // Nine seconds of unchanged assistant text plus no live thinking/status surface is
    // long enough to recover the observed stale Stop-button state without treating a
    // short reasoning pause as completion.
    if (stableMs >= 9000 && !transientStatusVisible()) {
      await suppressStaleStop(active, button, "stale-stop-stable-text", stableMs, 0);
    }
  }
