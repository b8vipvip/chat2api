(() => {
  ...
  async function ensureAudioV3(tabId) {
    await ensureContent(tabId);
    try {
      await chrome.scripting.executeScript({ target: { tabId }, files: ["voice_main.js"], world: "MAIN", injectImmediately: true });
    } catch (_) {}
    try {
      await chrome.scripting.executeScript({ target: { tabId }, files: ["content_voice_v2.js", "content_voice_fix_v3.js", "content_runtime_log.js"] });
    } catch (_) {}
    await sleep(150);
  }
  ...
  async function browserError(requestId, error) {
    await trySendSocket({ type: "image.error", kind: "voice", request_id: requestId, error: String(error?.message || error) });
  }

  handleServerMessage = async function handleAudioServerMessageV3(message) {
    ...
    try {
      const tab = await resolveAudioTab();
      await ensureAudioV3(tab.id);
      const prepared = await chrome.tabs.sendMessage(tab.id, { type: "chat2api.voice.trigger.prepare.v3", requestId: message.request_id, timeout_ms: 25000 });
      if (!prepared?.ok) throw new Error(prepared?.error || "Strict Voice trigger preflight failed");
      ...
      if (!response?.ok) throw new Error(response?.error || "Voice v2 controller rejected the request after v3 trigger preflight");
    } catch (error) {
