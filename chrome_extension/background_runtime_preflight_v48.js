(() => {
  ...
  const OVERLAY_FILES = [
    ...
    "content_network_stream_recovery_v55.js", "content_native_tool_stream_v63.js", "content_request_terminal_prompt_v88.js",
    "content_response_semantic_recovery_v51.js", "content_transient_retry_v50.js", "content_generation_liveness_v49.js",
    "content_bundle_marker_v71.js", "content_runtime_contract_v48.js", "content_runtime_contract_v71.js",
  ];
  const inflight = new Map();
  const state = {
    version: 71, revision: 111, required_bundle: REQUIRED_BUNDLE, required_revision: REQUIRED_REVISION,
    response_terminal_owner: "request-v6", native_tool_stream_revision: 63, multimodal_revision: 85,
    terminal_prompt_revision: 88, conversation_quota_failover_revision: 95, ui_hygiene_revision: 101,
    checks: 0, fast_path_hits: 0, contract_timeouts: 0, preflight_budget_exhausted: 0,
    hot_heals: 0, reloads: 0, reload_timeouts: 0, failures: 0, last: null,
  };
  ...
  async function sendMessageBounded(tabId, message, timeoutMs = CONTRACT_TIMEOUT_MS) {
    let timedOut = false;
    const timeout = sleep(timeoutMs).then(() => { timedOut = true; return null; });
  ...
  function current(result) {
    return Boolean(result?.ok && String(result?.marker?.bundle || "") === REQUIRED_BUNDLE &&
      ...
      result?.modules?.conversation_quota_failover_v95 && result?.modules?.ui_hygiene_v101);
  }
  ...
  async function heal(tabId, budgetMs = HOT_HEAL_BUDGET_MS) {
    ...
    await settleWithin(baseEnsureContent(tabId), Math.min(700, Math.max(150, budgetMs)), null);
    const injectBudget = Math.min(900, Math.max(150, budgetMs - (Date.now() - started)));
  ...
  async function preflight(tabId) {
    state.checks += 1; const started = Date.now(); let result = await contract(tabId);
    if (current(result)) {
      state.fast_path_hits += 1;
      const toolPreflight = await toolIsolationPreflight(tabId);
      await recordLast({tab_id: tabId, ok: true, mode: "current-fast-path-v87", response_terminal_owner: "request-v6",
        reloaded: false, hot_healed: false, marker: result.marker, modules: result.modules,
        contract_revision: result.contract_revision, native_tool_stream_revision: result.native_tool_stream_revision,
        multimodal_revision: result.multimodal_revision, terminal_prompt_revision: 88, conversation_quota_failover_revision: 95,
        ui_hygiene_revision: result.ui_hygiene_revision, tool_preflight: toolPreflight, elapsed_ms: Date.now()-started, at_ms: Date.now()});
      return true;
    }
    result = await heal(tabId); let reloaded = false; const hotHealed = current(result);
    if (hotHealed) state.hot_heals += 1;
    if (!hotHealed) {
      state.reloads += 1; reloaded = true;
      try { await chrome.tabs.reload(tabId, {bypassCache: true}); } catch (_) {}
      const reloadState = await waitForReloadOrContract(tabId, RELOAD_BUDGET_MS); result = reloadState.result;
    ...
    const toolPreflight = await toolIsolationPreflight(tabId);
    await recordLast({tab_id: tabId, ok: true, mode: reloaded ? "reload-repair-v87" : "hot-repair-v87",
      response_terminal_owner: "request-v6", reloaded, hot_healed: hotHealed, marker: result.marker, modules: result.modules,
      contract_revision: result.contract_revision, native_tool_stream_revision: result.native_tool_stream_revision,
      multimodal_revision: result.multimodal_revision, terminal_prompt_revision: 88, conversation_quota_failover_revision: 95,
      ui_hygiene_revision: result.ui_hygiene_revision, tool_preflight: toolPreflight, elapsed_ms: Date.now()-started, at_ms: Date.now()});
    return true;
  ...
  ensureContent = async function ensureCurrentWorkerRuntimeV71(tabId) {
    ...
    const task = preflight(id).finally(() => inflight.delete(id)); inflight.set(id, task); return task;
  };
})();