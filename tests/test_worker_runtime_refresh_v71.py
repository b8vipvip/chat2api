...
def test_worker_bundle_formally_seals_revisioned_content_epoch() -> None:
    ...
    assert "content_request_terminal_prompt_v88.js" in scripts
    ...
    assert scripts.index("content_request_v6.js") < scripts.index("content_network_stream_recovery_v55.js")
...
def test_programmatic_bootstrap_matches_current_text_request_chain() -> None:
    ...
    for name in ("multimodal_main_v78.js", "content_bundle_marker_v71.js", "content_multimodal_v78.js",
                 "content_rich_response_v69.js", "content_request_v6.js", "content_network_stream_recovery_v55.js",
                 "content_request_terminal_prompt_v88.js", "content_runtime_contract_v71.js"):
...
def test_runtime_preflight_hot_heals_before_reload() -> None:
    ...
    assert 'REQUIRED_BUNDLE = "0.22.96"' in preflight
    assert "REQUIRED_REVISION = 71" in preflight
    assert 'response_terminal_owner: "request-v6"' in preflight
    ...
    assert "content_request_terminal_prompt_v88.js" in preflight
    assert "result?.modules?.multimodal_v78" in preflight
    assert "result?.modules?.multimodal_main_v78" in preflight
    assert "result?.modules?.terminal_prompt_v88" in preflight
    assert "result = await heal(tabId)" in preflight
    assert "waitForReloadOrContract" in preflight


def test_v71_contract_preserves_runtime_checks_with_request_v6_terminal_owner() -> None:
    contract = (EXT / "content_runtime_contract_v71.js").read_text(encoding="utf-8")
    assert 'REQUIRED_BUNDLE = "0.22.96"' in contract
    assert "REQUIRED_REVISION = 71" in contract
    for token in ("__CHAT2API_REQUEST_CONTENT_V6__", "__CHAT2API_RICH_RESPONSE_V69__", "__CHAT2API_MULTIMODAL_V4__",
                  "__CHAT2API_REQUEST_TERMINAL_PROMPT_V88__", "terminal_prompt_v88", "multimodal_v78",
                  "data-chat2api-multimodal-main-v78", "__CHAT2API_NETWORK_STREAM_RECOVERY_V55__",
                  "data-chat2api-network-stream-parser", "__CHAT2API_TOOL_ISOLATION_V48__",
                  "__CHAT2API_RATE_LIMIT_CONTENT_V52__", "removeListener(prior.listener)"):
...
def test_runtime_preflight_v71_vm_regressions() -> None:
    completed = subprocess.run(["node", str(ROOT / "tests" / "runtime_preflight_refresh_v71.mjs")], cwd=ROOT,
                               check=False, capture_output=True, text=True, timeout=20)
    assert completed.returncode == 0, completed.stdout + completed.stderr
    assert "runtime preflight v71 single-owner regression scenarios passed" in completed.stdout
