...
def test_progress_details_gain_one_click_copy_without_changing_base_asset_file(tmp_path):
    ...
    for token in ('data-install-progress-detail-v2220="1"','data-copy-install-progress-v2220="1"','>复制详情</button>','const copyProgress = event.target.closest?.("[data-copy-install-progress-v2220]")','navigator.clipboard?.writeText','安装进度：${summary}','copyProgress.textContent = "已复制"'):
