...
def test_bootstrap_installs_v44_agent_and_fixed_online_upgrade_helper():
    ...
    patched = patch_upgrade_bootstrap(patch_initialize_bootstrap(patch_diagnostics_bootstrap(patch_install_ux_bootstrap(source))))
    ...
    assert 'echo "Worker Agent: 0.3.7 (支持后台一键更新 / 实时进度)"' in patched


def test_online_upgrade_helper_and_agent_shim_have_valid_syntax():
    shell = subprocess.run(["bash", "-n", str(ROOT / "scripts" / "linux_worker_upgrade.sh")], capture_output=True, text=True, check=False)
    assert shell.returncode == 0, shell.stderr
    python = subprocess.run(["python", "-m", "py_compile", str(ROOT / "scripts" / "linux_worker_agent_v44.py")], capture_output=True, text=True, check=False)
...
def test_worker_upgrade_ui_has_update_button_live_polling_and_one_time_compatibility_path():
    ...
    for token in ('button.textContent = "更新"', "/upgrade-status", "setTimeout(tick, 1000)", 'button.textContent = `更新 ${', "needs_bootstrap_once", "复制启用命令", "实时进度记录"):
        assert token in source
    result = subprocess.run(["node", "--check", str(ROOT / "app" / "admin_linux_worker_upgrade_v44.js")], capture_output=True, text=True, check=False)
...
def test_online_upgrade_control_plane_is_worker_authenticated_and_progress_survives_agent_restart():
    ...
    for token in ('@app.post("/api/workers/{worker_id}/upgrade-progress")','store.authenticate(worker_id, token)','@app.post("/api/admin/linux-workers/{worker_id}/upgrade")','"upgrade_worker"','needs_bootstrap_once','TARGET_AGENT_VERSION = "0.3.7"'):
        assert token in source
    ...
    assert 'LEGACY_SERVER_RUNTIME_VERSION = "0.22.62"' in runtime
