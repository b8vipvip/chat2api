...
def test_console_and_bridge_expose_live_capacity_controls() -> None:
    ...
    assert background_entry.index('"audio_routing_live.js"') < background_entry.index('"background_capacity_control_v35.js"')
...
def test_capacity_control_vm_contract() -> None:
    result = subprocess.run(
        ["node", str(ROOT / "tests" / "capacity_control_v35.mjs")],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stdout + result.stderr
    assert "capacity_control_v35 VM contract passed" in result.stdout


def test_capacity_control_javascript_syntax() -> None:
    result = subprocess.run(
        ["node", "--check", str(ROOT / "chrome_extension" / "background_capacity_control_v35.js")],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
