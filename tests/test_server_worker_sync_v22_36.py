...
def _worker(*, agent: str = TARGET_AGENT_VERSION, bundle: str = CHROME_BRIDGE_BUNDLE_VERSION) -> dict:
    return {
        ...
        "extension_client_id": "ext_test",
        "extension_device_id": "device_test",
        "hostname": "ubuntu03",
        "platform": "linux",
        "arch": "x86_64",
        "os_version": "Ubuntu",
        "agent_version": agent,
        "chrome_bridge_version": bundle,
        "metadata": {},
    }
...
def _coordinator(tmp_path: Path, *, worker: dict, online: bool = True):
    ...
    state = SimpleNamespace(
        ...
        linux_workers=store,
        worker_sockets={worker["worker_id"]: object()} if online else {},
        send_linux_worker_command=send,
    )
...
def _decision(required: bool):
    async def value(previous: str, target: str) -> dict:
        ...
        return {
            "known": True,
            "required": required,
            "changed_paths": ["chrome_extension/background_entry.js"] if required else ["app/playground_random_prompt_patch.py"],
            "worker_changed_paths": ["chrome_extension/background_entry.js"] if required else [],
            "compare_error": "",
            "truncated": False,
        }
...
def test_server_health_check_must_finish_before_worker_sync(tmp_path: Path) -> None:
    coordinator, _, calls = _coordinator(
        tmp_path,
        worker=_worker(bundle="0.8.7"),
        online=True,
    )
...
def test_stale_worker_is_upgraded_even_when_server_commit_is_server_only(tmp_path: Path) -> None:
    coordinator, store, calls = _coordinator(
        tmp_path,
        worker=_worker(bundle="0.8.7"),
        online=True,
    )
    coordinator._compare_decision = _decision(False)  # type: ignore[method-assign]
    result = asyncio.run(coordinator.run_once())
...
def test_worker_payload_change_forces_refresh_even_when_versions_match(tmp_path: Path) -> None:
    ...
    coordinator._compare_decision = _decision(True)  # type: ignore[method-assign]
    result = asyncio.run(coordinator.run_once())
    assert calls == [("wrk_test", "upgrade_worker")]
...
def test_runtime_and_entry_expose_auto_sync_contract() -> None:
    ...
    assert CHROME_BRIDGE_BUNDLE_VERSION == "0.8.15"
