...
def test_background_loads_single_route_authority_instead_of_legacy_request_reserving_router() -> None:
    ...
    assert '"conversation_dispatch.js"' in source
    ...
    assert "state.activeRequests = new Map()" not in router  # map is declared directly in the state literal
    assert "activeRequests: new Map()" in router
