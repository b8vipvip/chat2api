...
@pytest.mark.asyncio
async def test_brief_worker_disconnect_preserves_request_when_new_socket_reconnects(monkeypatch):
...
@pytest.mark.asyncio
async def test_worker_disconnect_becomes_terminal_after_grace_without_reconnect(monkeypatch):
...
@pytest.mark.asyncio
async def test_request_history_resolves_worker_to_device_code_name(tmp_path):
