    ...
    websocket.send_json(
        {
            ...
            "diagnostics": {"route": "chatgpt-voice"},
        }
    )
    websocket.send_json(
        {
        ...
        with client.websocket_connect(f"/ws/extensions/{client_id}?token={extension_token}") as websocket:
            ...
            def request():
                return client.post(
                    "/v1/audio/speech",
                    headers=headers(),
                    json={"model": "gpt-live", "input": "请说测试成功", "timeout": 30},
                )
    ...
    with TestClient(app_v6(tmp_path)) as client:
        upload = client.post(
            ...
            json={
                ...
                "data_base64": base64.b64encode(b"fake-user-audio" * 12).decode(),
                "purpose": "voice-input",
            },
        )
        ...
        with client.websocket_connect(f"/ws/extensions/{client_id}?token={extension_token}") as websocket:
            ...
            def request():
                return client.post(
                    ...
                    headers=headers(),
                    json={"model": "gpt-live", "audio_file_id": file_id, "timeout": 30},
                )
...
def test_managed_keys_gain_audio_scope(tmp_path: Path) -> None:
    with TestClient(app_v6(tmp_path)) as client:
        created = client.post(
            "/api/admin/keys",
            headers=headers(),
            json={"name": "Voice App", "expires_in_days": 7},
        )
