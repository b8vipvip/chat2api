...
def pair_extension(client: TestClient) -> tuple[str, str]:
    response = client.post(
        ...
        json={"name": "Chrome", "version": "0.7.0", "metadata": {"chatgpt_login_state": "ready", "chatgpt_login_composer_ready": True}},
    )
...
def test_extension_contains_visual_error_guard_voice_and_nonfocusing_routing() -> None:
    ...
    assert "chat2api.voice.request.v2" in audio_routing and "activateAudioTab" in audio_routing
    assert "dictation" not in audio_routing.lower()
    assert "chrome.windows.update" not in audio_routing
    assert "reuse-bound-tab" in image_routing and "restoreImageSession" in image_routing
    assert "chrome.windows.update" not in image_routing
    assert "duplicate_upload_dialog" in multimodal and "attachment_retry_suppressed" in multimodal
    assert "removeAttachmentsByName" in multimodal
    assert "uploadOne" in multimodal and "attachment_verified" in multimodal
    assert "cleanupPreviousAutomationDraft" in request_v3 and "enter-fallback" in request_v3
