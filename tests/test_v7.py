...
def test_dictation_rejects_non_audio_and_wrong_model(tmp_path: Path) -> None:
    with TestClient(app_v7(tmp_path)) as client:
        ...
        wrong_model = client.post(
            "/v1/audio/transcriptions",
            headers=headers(),
            json={"model": "gpt-live", "audio_file_id": file_id},
        )
        ...
        wrong_file = client.post(
            "/v1/audio/transcriptions",
            headers=headers(),
            json={"model": "gpt-dictation", "audio_file_id": file_id},
        )
...
def test_audio_router_prefers_voice_v2_and_exposes_dictation() -> None:
    root = Path(__file__).resolve().parents[1]
    ...
    assert "dictation.request" in routing
    assert "content_voice_v2.js" in routing
    assert "content_dictation.js" in routing
    ...
    assert "听写" in dictation
    assert "voice.mic.synthetic" in dictation
    assert "voice.input.play" in dictation
