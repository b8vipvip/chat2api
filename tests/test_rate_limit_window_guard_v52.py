...
def test_background_circuit_breaker_blocks_request_resolution_and_all_chatgpt_window_creates() -> None:
    ...
    for token in (
        ...
        'state.baseWindowsCreate',
        'chatgptCreateData(createData)',
        'await beforeWindowCreate("chrome.windows.create")',
        'chrome.windows.create = guardedCreate',
    ):
        assert token in source


def test_bundle_0812_load_order_and_runtime_contract_include_rate_limit_guard() -> None:
    ...
    assert 'REQUIRED_BUNDLE = "0.8.12"' in content_contract
