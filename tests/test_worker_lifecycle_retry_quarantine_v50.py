...
def test_runtime_versions_and_features() -> None:
    ...
    assert 'SERVER_RUNTIME_VERSION = "0.22.35"' in source
    assert 'CHROME_BRIDGE_BUNDLE_VERSION = "0.8.8"' in source
    for marker in (
...
def test_manifest_loads_lifecycle_and_retry_overlays_in_order() -> None:
    ...
    assert scripts.index("content_response_stream_recovery_v49.js") < scripts.index("content_transient_retry_v50.js")
...
def test_runtime_preflight_requires_v50_overlays() -> None:
