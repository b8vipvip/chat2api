...
def test_unpaired_extension_claims_worker_identity_without_pairing_code_and_ticket_cannot_replay(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        claimed = client.post(
            ...
            json={"ticket": ticket, "device_id": "device-fresh-123", "name": "Worker Chrome", "browser_name": "Chrome", "version": "0.22.96", "metadata": {"runtime_id": "runtime-test"}},
        )
        ...
        replay = client.post("/api/extensions/worker-bind", json={"ticket": ticket, "device_id": "device-fresh-123", "client_id": payload["client_id"], "client_token": payload["token"]})
...
def test_existing_extension_identity_is_reused_and_cannot_bind_to_two_active_workers(tmp_path):
    ...
    client_id, client_token = asyncio.run(app.state.registry.register("Existing", "Chrome", "0.22.96", {"device_id": "existing-device-123"}, device_id="existing-device-123"))
    ...
    with TestClient(app) as client:
        ...
        claimed = client.post("/api/extensions/worker-bind", json={"ticket": first_ticket,"device_id":"existing-device-123","client_id":client_id,"client_token":client_token,"version":"0.22.96"})
        assert claimed.status_code == 200
        ...
        conflict = client.post("/api/extensions/worker-bind", json={"ticket":second_ticket,"device_id":"existing-device-123","client_id":client_id,"client_token":client_token,"version":"0.22.96"})
...
def test_extension_binding_uses_session_storage_about_blank_scrub_and_no_pairing_secret():
    ...
    for token in ('chrome.storage.session.set({ [PENDING_KEY]: binding })','chrome.storage.session.remove(PENDING_KEY)','url.protocol !== "about:" || url.pathname !== "blank"','chrome.tabs.update(tabId, { url: "about:blank" })','/api/extensions/worker-bind','pairingCode: ""','linuxWorkerBindingVersion: 30','await restoreChatGpt(binding.tabId)'):
...
def test_binding_versions_are_aligned():
    ...
    assert '"version": "0.22.96"' in manifest
