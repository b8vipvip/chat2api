...
def test_host_updater_prefers_bounded_ssh_and_keeps_https_fallback():
    ...
    for token in (
        'GITHUB_SSH_URL="git@github.com:b8vipvip/chat2api.git"', 'HostName=ssh.github.com', 'Port=443',
        'HostKeyAlias=ssh.github.com', 'HostKeyAlias=github.com', 'BatchMode=yes',
        'ConnectTimeout=${GITHUB_SSH_CONNECT_SECONDS}', 'StrictHostKeyChecking=yes', 'HostKeyAlgorithms=ssh-ed25519',
        'GITHUB_HTTPS_URL="https://github.com/b8vipvip/chat2api.git"', '-c http.version=HTTP/1.1',
        '-c http.lowSpeedLimit=1024', 'http.lowSpeedTime=${GITHUB_HTTPS_LOW_SPEED_SECONDS}',
        ...
        'run_fetch_transport "ssh-443"', 'run_fetch_transport "ssh-22"', 'run_fetch_transport "https"',
        'fetch --prune "$GITHUB_SSH_URL" "$refspec"', 'fetch --prune "$GITHUB_HTTPS_URL" "$refspec"',
        'FETCH_TRANSPORT="$transport"', 'FETCH_ELAPSED_MS="$elapsed_ms"', 'FETCH_ATTEMPTS="$((FETCH_ATTEMPTS + 1))"',
        '通道=${FETCH_TRANSPORT}', '耗时=${FETCH_ELAPSED_MS}ms',
    ):
...
def test_transport_release_contract_and_shell_syntax():
    ...
    for filename in ("scripts/chat2api_server_update.sh", "scripts/install_chat2api_server_updater.sh"):
        result = subprocess.run(["bash", "-n", str(ROOT / filename)], capture_output=True, text=True, check=False)
