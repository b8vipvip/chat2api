...
def test_manifest_requires_fresh_document_marker_and_passive_recovery():
    ...
    assert manifest["version"] == CHROME_BRIDGE_BUNDLE_VERSION == "0.8.17"
...
def test_background_preflight_wraps_final_conversation_dispatch():
    ...
    assert 'REQUIRED_BUNDLE = "0.8.17"' in legacy_contract
    ...
    assert 'bundle: "0.8.17"' in legacy_marker
...
def test_runtime_contract_exposes_v48_v49_v50_v51_and_v55_features():
    ...
    assert SERVER_RUNTIME_VERSION == "0.22.45"
    assert payload["chrome_bridge"]["bundle_version"] == "0.8.17"
