...
def test_manifest_requires_fresh_document_marker_and_passive_recovery():
    ...
    assert manifest["version"] == CHROME_BRIDGE_BUNDLE_VERSION == "0.8.36"
...
def test_background_preflight_wraps_single_route_dispatch_without_legacy_window_owners():
    ...
    assert 'REQUIRED_BUNDLE = "0.8.36"' in preflight
    ...
    assert 'REQUIRED_BUNDLE = "0.8.36"' in legacy_contract
    assert 'REQUIRED_BUNDLE = "0.8.36"' in contract
    ...
    assert 'bundle: "0.8.36"' in legacy_marker
    assert 'bundle: "0.8.36"' in marker
...
def test_runtime_contract_exposes_v48_v49_v50_v51_and_v55_features():
    ...
    assert payload["chrome_bridge"]["bundle_version"] == "0.8.36"
