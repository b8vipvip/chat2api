(() => {
  ...
  function installView() {
    ...
    let button = nav.querySelector('[data-view="window-manager"]');
    if (!button) {
    ...
    if (button.dataset.chat2apiWindowNavV93 !== "1") {
      button.dataset.chat2apiWindowNavV93 = "1";
  ...
  async function refresh(force = false) {
    ...
    finally { if (state.refreshPromise === task) state.refreshPromise = null; }
  }
  ...
  // v93 authority boundary: Window Management owns only #view-window-manager.
  // Request IDs are rendered by admin_request_history_v93.js. This module never
  // observes or writes #rqBody.
