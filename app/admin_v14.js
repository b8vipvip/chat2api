(() => {
  ...
  if (brandSmall) brandSmall.textContent = "Server Console · v0.14.1";
  ...
  if (docs && !docs.querySelector("[data-chat2api-v14-time]")) {
    ...
    block.innerHTML = `
      ...
      <h2>模型切换验证</h2>
      <p>扩展会优先使用当前 composer 的被动 DOM 状态验证模型与推理强度。模型菜单二次打开无法显示已选模型时，如果 composer 已明确显示例如 <code>5.5 高</code>，允许通过被动状态恢复验证；GPT-5.6 Sol 若在成功切换后把组合控件从 <code>5.5 极速</code> 收缩为仅 <code>极速</code>，扩展会结合“已知旧 family + 精确目标 family 点击 + 控件发生可观察变化”恢复 family 状态。</p>
