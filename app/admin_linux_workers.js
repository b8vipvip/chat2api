(() => {
  ...
  loginDialog.innerHTML = `<div style="padding:14px"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px"><div><div style="font-size:18px;font-weight:700">远程登录 ChatGPT</div><div id="linuxLoginWorkerName" style="margin-top:3px;color:#94a3b8"></div></div><div style="display:flex;align-items:center;gap:10px"><span id="linuxLoginStatus" style="color:#94a3b8">准备中…</span><button class="action" id="closeLinuxLogin" type="button">结束登录</button></div></div><div style="font-size:12px;color:#94a3b8;margin-bottom:10px">这是服务器 Xvfb :99 中真实 Chrome 画面。打开此窗口前系统已经实时验证当前代理可以访问 ChatGPT。请直接在画面里输入账号、密码、验证码或完成 CAPTCHA；chat2api 不保存这些输入内容。点击画面后即可键盘输入，也可以直接按 Ctrl+V（macOS 为 Command+V）把本机剪贴板中的单行文本粘贴到远程当前输入框。登录状态确认后本窗口会自动结束。</div><div id="linuxLoginViewport" style="position:relative;background:#000;border:1px solid #334155;border-radius:8px;overflow:hidden;min-height:360px;display:flex;align-items:center;justify-content:center"><img id="linuxLoginFrame" draggable="false" alt="远程 Chrome 画面" style="display:block;max-width:100%;max-height:calc(100vh - 210px);outline:none;cursor:default;user-select:none"><textarea id="linuxLoginKeyboardSink" aria-label="远程键盘输入" autocomplete="off" autocapitalize="off" spellcheck="false" style="position:absolute;left:-10000px;top:0;width:1px;height:1px;opacity:0;pointer-events:none;resize:none"></textarea><div id="linuxLoginPlaceholder" style="position:absolute;color:#94a3b8">正在连接 Worker 画面…</div></div></div>`;
  ...
  const queueRemoteText = raw => {
    let text = String(raw ?? "").replace(/\r\n?/g, "\n").replace(/\n+$/g, "");
    if (!text) return;
    if (text.includes("\n") || /[\u0000-\u001f\u007f]/.test(text)) { setLoginStatus("为避免误提交，远程粘贴只支持单行文本"); return; }
    const characters = Array.from(text);
    if (characters.length > 512) { setLoginStatus("远程粘贴文本过长 · 最多 512 个字符"); return; }
    setLoginStatus("正在把本机剪贴板文本粘贴到远程输入框…");
    for (const character of characters) queueRemoteInput({kind:"key",key:character,modifiers:[]});
    inputChain = inputChain.then(() => setLoginStatus("已完成远程粘贴 · 可继续输入"));
  };
  ...
  const keyboardSink = document.getElementById("linuxLoginKeyboardSink");
  remoteImage.addEventListener("click", event => { keyboardSink.focus({preventScroll:true}); const point = remotePoint(event); if (!point) return; queueRemoteInput({kind:"mouse",action:"click",button:event.button+1,...point}); });
  remoteImage.addEventListener("contextmenu", event => { event.preventDefault(); keyboardSink.focus({preventScroll:true}); const point = remotePoint(event); if (!point) return; queueRemoteInput({kind:"mouse",action:"click",button:3,...point}); });
  ...
  keyboardSink.addEventListener("paste", event => { if (!loginTicket) return; const text = event.clipboardData?.getData("text/plain") || ""; if (!text) return; event.preventDefault(); keyboardSink.value = ""; queueRemoteText(text); });
  keyboardSink.addEventListener("keydown", event => { if (!loginTicket) return; const isPaste=(event.ctrlKey||event.metaKey)&&!event.altKey&&String(event.key||"").toLowerCase()==="v"; if(isPaste)return; const modifiers=[]; if(event.ctrlKey)modifiers.push("ctrl");if(event.altKey)modifiers.push("alt");if(event.shiftKey)modifiers.push("shift");if(event.metaKey)modifiers.push("super"); const supportedSpecial=new Set(["Enter","Tab","Escape","Backspace","Delete","ArrowLeft","ArrowRight","ArrowUp","ArrowDown","Home","End","PageUp","PageDown"," "]); if(event.key.length!==1&&!supportedSpecial.has(event.key))return; event.preventDefault(); queueRemoteInput({kind:"key",key:event.key,modifiers}); });
