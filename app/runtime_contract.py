...
def version_contract_payload(app: FastAPI) -> dict[str, Any]:
    ...
    return {
        ...
        "chrome_bridge": {"version": CHROME_BRIDGE_VERSION},
...
def _admin_version_script() -> str:
  ...
  function patchBrand() {{ const node = document.querySelector(".brand small"); if (node && node.textContent !== BRAND) node.textContent = BRAND; }}
  function patchStatusVersion() {{ const node = document.getElementById("status"); if (!node) return; const value = String(node.textContent || "").trim(); if (VERSION_ONLY.test(value) && value !== LABEL) node.textContent = LABEL; }}
  function patchVersion() {{ document.documentElement.dataset.chat2apiRuntimeVersion = VERSION; patchBrand(); patchStatusVersion(); }}
  const baseShow = typeof globalThis.show === "function" ? globalThis.show : null;
  if (baseShow && !baseShow.__chat2apiRuntimeVersionOwner) {{ const wrappedShow = async (...args) => {{ const result = await baseShow(...args); patchVersion(); return result; }}; wrappedShow.__chat2apiRuntimeVersionOwner = true; globalThis.show = wrappedShow; }}
  const observeVersionNode = node => {{ if (!node || typeof MutationObserver !== "function") return; new MutationObserver(() => patchVersion()).observe(node, {{childList:true,characterData:true,subtree:true}}); }};
  patchVersion(); observeVersionNode(document.querySelector(".brand small")); observeVersionNode(document.getElementById("status")); setTimeout(patchVersion,150); setTimeout(patchVersion,650);
}})();\n'''
...
def install_runtime_contract(app: FastAPI) -> FastAPI:
    ...
    async def runtime_contract_response(request: Request, call_next):
        ...
        if path in {"/admin", "/developers"} and "text/html" in content_type:
            ...
            if version_marker not in text: text = text.replace("</body>", version_marker + "</body>")
            if path == "/admin":
                workers_marker = f'<script src="{ADMIN_LINUX_WORKERS_ASSET}"></script>'
                if workers_marker not in text: text = text.replace("</body>", workers_marker + "</body>")
                columns_marker = f'<script src="{ADMIN_EXTENSION_COLUMNS_ASSET}"></script>'
                if columns_marker not in text: text = text.replace("</body>", columns_marker + "</body>")
        ...
        if path.startswith("/api/admin/") and "application/json" in content_type:
            ...
            except Exception: return Response(raw, status_code=response.status_code, media_type="application/json")
            if isinstance(payload, dict) and (path == "/api/admin/overview" or "version" in payload): payload["version"] = SERVER_RUNTIME_VERSION
            headers = {key:value for key,value in response.headers.items() if key.lower() not in {"content-length","content-type"}}
