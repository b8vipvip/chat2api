(() => {
  const KEY = "__CHAT2API_WORKER_PRESENTATION_V65__";
  if (globalThis[KEY]) return;

  const VERSION = 65;
  // Keep the v64 preference key so existing column visibility/order choices survive the hotfix.
  ...
  // DOM ordering must be convergent. v64 unconditionally reinserted nodes that
  // were already in the correct position. Because the table/header are watched
  // by MutationObserver, those no-op moves generated another childList mutation,
  // producing an endless observer -> microtask -> DOM move loop that could starve
  // the admin console. Only mutate when a node is genuinely out of position.
  function placeExtraNodes(parent, order) {
    if (!parent) return;
    for (const key of EXTRAS.map(item => item.key)) {
      const children = [...parent.children];
      const node = children.find(child => cellKey(child) === key);
      if (!node) continue;
      const index = order.indexOf(key);
      if (index < 0) continue;
      const nextKey = order.slice(index + 1).find(candidate => children.some(child => cellKey(child) === candidate));
  ...
  function ensureHeader() {
    ...
    for (const item of EXTRAS) {
      ...
      th.dataset.chat2apiPresentationV65 = "1";
    ...
    for (const tr of body.rows) {
      ...
      for (const item of EXTRAS) {
        ...
        if (!td) {
          ...
          td.dataset.chat2apiPresentationV65 = "1";
          tr.appendChild(td);
        }
  ...
  function pairingRows(rows) {
    ...
    [...body.rows].forEach((tr, index) => {
      ...
      tr.dataset.v65PairingId = id;
      tr.dataset.v65PairingName = String(row.name || "");
      const actions = tr.querySelector(".rowactions");
      if (!actions || actions.querySelector("[data-v65-pairing-rename]")) return;
      const button = document.createElement("button");
      button.className = "action";
      button.dataset.v65PairingRename = "1";
  ...
  function extraCard(item, prefs) {
    ...
    card.dataset.v65ExtraCard = item.key;
    ...
    card.innerHTML = `<div style="display:flex;align-items:center;gap:9px"><span data-v65-order-number style="min-width:30px;padding:3px 6px;text-align:center;border-radius:999px;background:rgba(59,130,246,.16);color:#93c5fd;font-size:11px">00</span><label style="display:flex;align-items:center;gap:8px;flex:1;font-weight:600"><input type="checkbox" data-v65-column-visible="${item.key}" ${prefs.visible[item.key] !== false ? "checked" : ""}><span>${item.label}</span></label></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><button class="action" data-v65-column-move="${item.key}" data-delta="-1">← 前移</button><button class="action" data-v65-column-move="${item.key}" data-delta="1">后移 →</button></div>`;
    return card;
  }
  ...
  function patchSettingsMenu() {
    ...
    const prefs = loadPrefs();
    for (const item of EXTRAS) {
      let card = grid.querySelector(`[data-v65-extra-card="${item.key}"]`);
      if (!card) {
        card = extraCard(item, prefs);
        grid.appendChild(card);
      }
      const checkbox = card.querySelector("[data-v65-column-visible]");
    ...
    for (const key of order) {
      const card = EXTRA_KEYS.has(key)
        ? grid.querySelector(`[data-v65-extra-card="${key}"]`)
        : cardForBaseKey(grid, key);
      if (card) cards.set(key, card);
    }
    const desiredCards = order.map(key => cards.get(key)).filter(Boolean);
    ...
    [...grid.children].forEach((card, index) => {
      const badge = card.querySelector("[data-v65-order-number]") || card.firstElementChild?.querySelector("span");
      if (badge) badge.textContent = String(index + 1).padStart(2, "0");
    });

    for (const item of EXTRAS) {
      const card = grid.querySelector(`[data-v65-extra-card="${item.key}"]`);
  ...
  async function refresh(force = false) {
    ...
    const task = (async () => {
      ...
      console.warn("chat2api worker presentation v65 refresh failed", error);
      return null;
    });
    refreshTask = task;
  ...
  document.addEventListener("change", event => {
    const input = event.target?.closest?.("[data-v65-column-visible]");
    if (!input) return;
    const key = String(input.dataset.v65ColumnVisible || "");
  ...
  document.addEventListener("click", async event => {
    const move = event.target?.closest?.("[data-v65-column-move]");
    if (move) {
      event.preventDefault();
      event.stopPropagation();
      moveExtra(String(move.dataset.v65ColumnMove || ""), Number(move.dataset.delta || 0));
      return;
    }
    const rename = event.target?.closest?.("[data-v65-pairing-rename]");
    if (rename) {
      event.preventDefault();
      event.stopPropagation();
      ...
      try {
        ...
        await refresh(true);
        const row = document.querySelector(`#pairingBody tr[data-v65-pairing-id="${CSS.escape(pairingId)}"]`);
