...
def _canonical_done_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    Codex's own Responses test server emits function/custom/tool-search calls in
    response.output_item.done without synthetic item ids or lifecycle status.
    Keep ids on the optional added/delta lifecycle for UI correlation, but make
    the authoritative done item byte-shape compatible with that contract.
    """
    done = dict(item)
...
def _mark_tool_follow_up(response: dict[str, Any], tool_items: list[dict[str, Any]]) -> None:
    if tool_items:
        # Codex already infers follow-up from a parsed tool item. Explicit false
        # also protects older/lite clients from treating a tool-call response as
        # a terminal assistant turn when the provider omits this optional field.
        response["end_turn"] = False
        response.setdefault("metadata", {})["chat2api_tool_stream"] = "responses-v112-codex-0149"


async def _emulated_stream_v112(
    app: FastAPI,
    request: Request,
    state: Any,
    *,
    ...
    try:
        ...
        if not tool_items:
            completed.setdefault("metadata", {})["chat2api_tool_stream"] = "responses-v112-codex-0149"
...
def install_responses_tool_stream_v112_patch(app: FastAPI) -> FastAPI:
    if getattr(app.state, "responses_tool_stream_v112_installed", False):
        return app
    v109._emulated_stream = _emulated_stream_v112
