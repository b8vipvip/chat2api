...
def _load(path: Path) -> dict[str, Any]:
    ...
    result["default_worker_concurrency"] = _limit(raw.get("default_worker_concurrency"), DEFAULT_WORKER_CONCURRENCY)
    result["default_reserve_windows"] = _limit(raw.get("default_reserve_windows"), DEFAULT_RESERVE_WINDOWS)
    result["default_key_concurrency"] = _limit(raw.get("default_key_concurrency"), DEFAULT_KEY_CONCURRENCY)
    workers: dict[str, dict[str, int]] = {}
    for client_id, row in (raw.get("workers") or {}).items():
        if not isinstance(row, dict):
            continue
        workers[str(client_id)] = {
            "max_concurrency": _limit(row.get("max_concurrency"), result["default_worker_concurrency"]),
            "reserve_windows": _limit(row.get("reserve_windows"), result["default_reserve_windows"]),
        }
    keys = {
...
def _save(path: Path, config: dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "keys": {str(key_id): int(value) for key_id, value in sorted(config["keys"].items())},
    }
...
def _save_reserve_compat(runtime: dict[str, Any]) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    default_limit = _limit(runtime.get("default_max_concurrency"), DEFAULT_RESERVE_WINDOWS)
    client_limits = runtime.get("client_limits") if isinstance(runtime.get("client_limits"), dict) else {}
    tmp.write_text(
        json.dumps(
            {
                "version": 2,
                ...
                "clients": {str(client_id): _limit(value, default_limit) for client_id, value in sorted(client_limits.items())},
                "request_weight": 1,
            },
            ensure_ascii=False,
            indent=2,
...
def install_capacity_queue_v57_patch(app: FastAPI) -> FastAPI:
    ...
    def worker_settings(client_id: str) -> dict[str, int]:
        ...
        return {
            "max_concurrency": _limit(row.get("max_concurrency"), config["default_worker_concurrency"]),
            "reserve_windows": _limit(row.get("reserve_windows"), config["default_reserve_windows"]),
        }
    ...
    def key_limit(key_id: str | None) -> int:
        key_id = str(key_id or "anonymous")
        return _limit(config["keys"].get(key_id), config["default_key_concurrency"])
    ...
    def is_rate_limited(client_id: str) -> tuple[bool, float]:
        remaining = max(0.0, float(cooldown_until.get(str(client_id), 0.0)) - time.time())
    ...
    def capacity_snapshot(client_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "account_type": str((getattr(registry.clients.get(client_id), "metadata", {}) or {}).get("account_type") or "unknown"),
    ...
    async def create_fifo(request_id: str, client_id: str):
        ...
        client_id = str(client_id)
        key_id = str(registry.routing_key_context.get() or "anonymous")
        started = time.perf_counter()
        wq = worker_queues[client_id]
        kq = key_queues[key_id]
        wq.append(request_id)
        kq.append(request_id)
        try:
            async with condition:
                while True:
                    if client_id not in registry.online_client_ids():
                        raise RuntimeError("selected extension went offline while waiting in the capacity queue")
                    limited, remaining = is_rate_limited(client_id)
                    worker_ready = bool(wq and wq[0] == request_id and used_units(client_id) < worker_limit(client_id))
                    key_ready = bool(kq and kq[0] == request_id and key_active[key_id] < key_limit(key_id))
                ...
                if wq and wq[0] == request_id:
                    wq.popleft()
                if kq and kq[0] == request_id:
                    kq.popleft()
            ...
            try:
                wq.remove(request_id)
            except ValueError:
                pass
            try:
                kq.remove(request_id)
            except ValueError:
                pass
            async with condition:
                condition.notify_all()
    ...
    async def release_fifo(request_id: str) -> None:
        ...
        async with condition:
            if key_id:
                key_active[key_id] = max(0, int(key_active.get(key_id, 0)) - 1)
            condition.notify_all()

    def _rate_limit_seconds(message: str) -> int:
    ...
    async def publish_rate_aware(request_id: str, event: dict[str, Any]) -> bool:
            ...
            if any(token in lowered for token in RATE_LIMIT_PATTERNS):
                ...
                if state:
                    cooldown_until[state.client_id] = max(
                        ...
                        time.time() + _rate_limit_seconds(message),
                    )
                    async with condition:
                        condition.notify_all()
        return await base_publish(request_id, event)

    async def send_with_worker_limit(client_id: str, message: dict[str, Any]) -> None:
        if isinstance(message, dict) and str(message.get("type") or "") in {
            "chat.request", "image.request", "voice.request", "voice.live.request"
        }:
    ...
    def summaries_v57() -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            row["default_max_concurrency"] = int(config["default_worker_concurrency"])
    ...
    async def capacity_admin_state() -> dict[str, Any]:
        workers = {
            str(client_id): {
                **worker_settings(client_id),
                ...
                "queued": len(worker_queues.get(str(client_id), ())),
        ...
        return {
            ...
            "defaults": {
                ...
                "api_key_max_concurrency": int(config["default_key_concurrency"]),
            },
            "workers": workers,
    ...
    async def update_worker_capacity(client_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        maximum = _limit(body.get("max_concurrency"), DEFAULT_WORKER_CONCURRENCY)
        reserve = _limit(body.get("reserve_windows"), DEFAULT_RESERVE_WINDOWS)
        config["workers"][client_id] = {"max_concurrency": maximum, "reserve_windows": reserve}
        await persist_reserve(client_id, reserve)
        return {"ok": True, "client_id": client_id, **worker_settings(client_id)}

    @app.put("/api/admin/keys/{key_id}/concurrency-v57")
    async def update_key_capacity(key_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        await persist()
        return {"ok": True, "key_id": key_id, "max_concurrency": maximum, "active": int(key_active.get(key_id, 0))}
